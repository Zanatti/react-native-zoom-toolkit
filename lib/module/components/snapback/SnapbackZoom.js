...
const SnapbackZoom = ({
  ...
  const tap = Gesture.Tap().withTestId('tap').enabled(gesturesEnabled).maxDuration(110).maxDelay(80).maxDistance(18).numberOfTaps(1).minPointers(1).runOnJS(true).onEnd(e => onTap === null || onTap === void 0 ? void 0 : onTap(e));
  ...
  const zoomPan = Gesture.Pan().withTestId('zoomPan').enabled(gesturesEnabled).maxPointers(1).minDistance(5).onStart(() => {
  ...
  const pan = Gesture.Pan().withTestId('pan').enabled(gesturesEnabled).maxPointers(1).minDistance(10).activeOffsetY([-24, 24]).failOffsetX([-16, 16]).onBegin(() => {
    'worklet';
  ...
  const composedGesture = Gesture.Simultaneous(pinch, Gesture.Race(zoomPan, pan, composedTapGesture));
