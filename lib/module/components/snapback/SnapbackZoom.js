...
const SnapbackZoom = ({
  ...
  const tap = Gesture.Tap().withTestId('tap').enabled(gesturesEnabled).maxDuration(50).numberOfTaps(1).minPointers(1).runOnJS(true).onEnd(e => onTap === null || onTap === void 0 ? void 0 : onTap(e));
  ...
  const pan = Gesture.Pan().withTestId('pan').enabled(gesturesEnabled).maxPointers(1).minDistance(80) // Require significant movement
  ...
  return /*#__PURE__*/React.createElement(GestureDetector, {
    gesture: Gesture.Simultaneous(composedTapGesture, Gesture.Race(pinch, pan))
