...
const SnapbackZoom = ({
  ...
  const tap = _reactNativeGestureHandler.Gesture.Tap().withTestId('tap').enabled(gesturesEnabled).maxDuration(50).numberOfTaps(1).minPointers(1).runOnJS(true).onEnd(e => onTap === null || onTap === void 0 ? void 0 : onTap(e));
  ...
  const pan = _reactNativeGestureHandler.Gesture.Pan().withTestId('pan').enabled(gesturesEnabled).maxPointers(1).minDistance(80) // Require significant movement
  ...
  return /*#__PURE__*/_react.default.createElement(_reactNativeGestureHandler.GestureDetector, {
    gesture: _reactNativeGestureHandler.Gesture.Simultaneous(composedTapGesture, _reactNativeGestureHandler.Gesture.Race(pinch, pan))
