...
const SnapbackZoom = ({
  ...
  const isZoomed = (0, _reactNativeReanimated.useSharedValue)(false);
  ...
  const zoomPan = _reactNativeGestureHandler.Gesture.Pan().withTestId('zoomPan').enabled(gesturesEnabled).maxPointers(1).minDistance(5).onStart(() => {
  ...
  const composedGesture = _reactNativeGestureHandler.Gesture.Simultaneous(pinch, _reactNativeGestureHandler.Gesture.Race(zoomPan, pan, composedTapGesture));
